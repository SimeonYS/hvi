...
class HviPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('hvi.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `hvi`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM hvi WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO hvi (date, title, link, content)"
